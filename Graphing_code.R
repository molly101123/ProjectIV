...
##### The Node Bond Percolation case
# firstly we will consider fixed values of $p_s$

shortcut_threshold_nb <- function(k,p_s,p_b){
  phi <- (-9040* p_b**3* p_s**3 - 480* p_b**3 *p_s**2 + 926* p_b**2* p_s**2 + 24* p_b**2 *p_s
          - 44 *p_b *p_s + 1)/(2*k*p_b*p_s*(- 550* p_b**3 *p_s**3 - 30* p_b**3* p_s**2 + 763* p_b**2* p_s**3 + 27 *p_b**2* p_s**2 + p_b**2 *p_s - 40* p_b *p_s**2 + p_s))
  return(phi)
}

shortcut_threshold_nb(2,0.03,0.2)
# again note how this is not dependent on L - except that we take L to be large

p_b <- seq(0.01,0.1,length.out = 30)
# most of these are high in the critical case so I'll start with a range from 
# 0 to 1 about 16 long
plot(p_b)
length <- length(p_b)
...
for (i in 1:length) {
  phis[[i]] <- shortcut_threshold_nb(k,p_s,p_b[i])
}
...
x <- seq(0.015,0.02,length.out = 30)

plot(x,phis_ps_0.01,ylab = 'Shortcut threshold',
     xlab = 'Transmission probability', ylim = c(0,max(170000))) 
lines(x,phis_ps_0.01)
points(x,phis_ps_0.02, pch = 2)
lines(x,phis_ps_0.02)
points(x,phis_ps_0.03, pch = 5)
lines(x,phis_ps_0.03)
